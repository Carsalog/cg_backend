...
module.exports = async function (req, res, next) {
  ...
  if (!(await State.getById(req.body.state))) return res.status(404).send({error: "Cannot find this state"});
  if (!(await City.getById(req.body.city))) return res.status(404).send({error: "Cannot find this city"});
  ...
  if (!user.su && user._id !== post.author._id) return res.status(403).send({error: "You cannot edit this post"});
